...
easyModbus2::easyModbus2(int a)
{
    // int a -- don't use this, just for differentiate the first construct function
    rtuOrAscii = false;  // start the ascii mode
    ...
    for(auto sPortName:mPortNameList){
        ...
        {
            // Matching Serial Port
            sendMsg(matchMessageAA);
            QByteArray receiveByte = readMsg();
            QString receivedMsg = receiveByte.toHex();
            qDebug() << QObject::tr("ReceivedMsg: %1").arg(receivedMsg) << endl;
            if (receivedMsg == matchMessageAB)
            {
                qDebug() << QObject::tr("Successful Opening of Serial Port: %1").arg(serialPortName) << endl;
                matchingFlag = true;
                break;
...
void easyModbus2::sendMsg(QString input)
{
    ...
    //        qDebug() << QObject::tr("data %1").arg(data) << endl;
            writeData.append(data);
        }

        // qDebug() << QObject::tr("write %1").arg(writeData) << endl;

        ////发送数据
        //	serialPort.write(buf);
    //    QByteArray str1 = QByteArray::fromHex(str.toLatin1().data());
    //    for(int i = 0; i < str1.size();i++){
    //        qDebug() << QObject::tr("str1 %1").arg(str1[i]) << endl;
    //    }
...
void easyModbus2::sendMsg(int intInput)
{
    if(rtuOrAscii){
        ...
        QString input = QString::fromStdString(LRC::convertIntToModbusString(intInput));
        sendMsg(input);
    }
}
