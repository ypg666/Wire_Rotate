...
TIS_Camera2 *TIS_Camera2::_instance = 0;

TIS_Camera2::TIS_Camera2()
{
...
void TIS_Camera2::Camera(QWidget *win)
{
    if (Grab1.isLive())
    {
        Grab1.stopLive();
...
//    Grab1.removeListener(pListener1);
    Grab1.removeListener(Listener2::Instance());
    if (Grab1.isDevValid())//
    {
...
void TIS_Camera2::Trigger(QWidget *win)
{
    if (Grab1.isLive())
    {
        Grab1.stopLive();
...
//    Grab1->showDevicePage();//bool
    ...
    {
       // pListener1 = new Listener1;
        Grab1.addListener(Listener2::Instance(),DShowLib::GrabberListener::eFRAMEREADY);//
        pSink = FrameHandlerSink::create( eRGB32,3);//黑白相机用eY800，彩色用eRGB32格式
        // Disable snap mode.
        pSink->setSnapMode(false);
        // Set the sink.
...
//相机初始化
void TIS_Camera2::CameraInit()
{
    Grab1.showDevicePage();                                             //显示相机设备
    Grab1.saveDeviceStateToFile("device2.xml");
    Grab1.loadDeviceStateFromFile("device2.xml",true);
//    Grab1.startLive();
    emit ini();
}
/*
void TIS_Camera2::CameraShowImage()//实时显示图片
{
...
*/
QImage TIS_Camera2::cvMat2QImage(const cv::Mat& mat, bool clone, bool rb_swap)
{
